...
api.delete("/materials/:id", async (c) => {
  await c.env.DB.prepare(`UPDATE materials SET status = 'archive' WHERE id = ?`)
    .bind(c.req.param("id"))
    .run();
  return c.json({ ok: true });
});
...
api.delete("/materials/:id/permanent", async (c) => {
  await c.env.DB.prepare(`DELETE FROM materials WHERE id = ?`)
    .bind(c.req.param("id"))
    .run();
  return c.json({ ok: true });
});
...
api.delete("/topics/:id", async (c) => {
  ...
  await c.env.DB.prepare(`DELETE FROM discussion_rooms WHERE topic_id = ?`)
    .bind(id)
    .run();
  await c.env.DB.prepare(`DELETE FROM publications WHERE topic_id = ?`)
    .bind(id)
    .run();
...
api.delete("/rooms/:id", async (c) => {
  ...
  await c.env.DB.prepare(`DELETE FROM messages WHERE room_id = ?`)
    .bind(id)
    .run();
  await c.env.DB.prepare(`DELETE FROM publications WHERE room_id = ?`)
    .bind(id)
    .run();
  await c.env.DB.prepare(`DELETE FROM discussion_rooms WHERE id = ?`)
    .bind(id)
    .run();
  return c.json({ ok: true });
});
...
api.delete("/publications/:id", async (c) => {
  await c.env.DB.prepare(`DELETE FROM publications WHERE id = ?`)
    .bind(c.req.param("id"))
    .run();
  return c.json({ ok: true });
});
...
api.patch("/admin/users/:id", async (c) => {
  ...
  await c.env.DB.prepare(`UPDATE users SET ${set} WHERE id = ?`)
    .bind(...vals, id)
    .run();
  return c.json({ ok: true });
});
...
api.post("/rooms/:id/messages", async (c) => {
  ...
  const result = await c.env.DB.prepare(
    ...
    .bind(roomId, user_id || null, text.trim())
    .run();
