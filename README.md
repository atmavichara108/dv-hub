...
**Комнаты дискуссий** — создание из темы или отдельно. Дата и время, участники, self-hosted видеозвонок через MiroTalk (meet.re-search.wiki) прямо на странице, чат с поддержкой @упоминаний и #тем, заметки, задачи, смена статуса.
...
**Роли** — admin, member, guest. Управление через админку.
...
`context/` — это git submodule на репо [dv-project](https://github.com/atmavichara108/dv-project) с операционным контекстом движения (vision, принципы, структура, бэклог задач). Формат: [OKF v0.1](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) — каждый файл содержит YAML frontmatter с полем `type` документа. Используется AI-агентами и контрибьюторами для понимания продуктовых решений.
...
Контекст движения (`context/`) хранится в формате [OKF v0.1](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md). Каждый документ содержит YAML frontmatter с полем `type` для автоматической обработки агентами.
